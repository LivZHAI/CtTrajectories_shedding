# read fit_posteriors_b117.stan and compile it to C++ 
...
ct_fit <- sampling(ct_model, 
	data=list(
		N=nrow(indiv_data), 
		n_id=length(unique(indiv_data$id)),
		lod=global_pars[["lod"]], 
		id=indiv_data$id,
...
shared_params_df <- make_shared_params_df(params, c("dpmeanW","wpmeanW","wrmeanW","dpsd","wpsd","wrsd")) 
